...
import { createPatientResourceAction, updatePatientResourceAction, deletePatientResourceAction } from '../actions';
import Button from '@/components/ui/Button';
import { cmsRecordLabel, type CmsListRecord } from '@/features/cms/management-types';

export default function PatientResourceManagement({ initialData }: { initialData: CmsListRecord[] }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [error, setError] = useState('');
  
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<CreatePatientResourceInput | UpdatePatientResourceInput>({
    resolver: zodResolver(editingId ? UpdatePatientResourceSchema : CreatePatientResourceSchema),
    defaultValues: { title: '', description: '' },
  });

  const onSubmit = async (data: CreatePatientResourceInput) => {
    setError('');
    let res;
    if (editingId) {
      res = await updatePatientResourceAction({ ...data, id: editingId });
    } else {
      res = await createPatientResourceAction(data);
    }
    if (res.success) {
      setEditingId(null);
  ...
  const editRecord = (record: CmsListRecord) => {
    ...
    reset({ ...record } as unknown as CreatePatientResourceInput);
  };

  const deleteRecord = async (id: string) => {
    if (confirm('Are you sure you want to delete this record?')) {
      await deletePatientResourceAction(id);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4 border rounded bg-white">
        <h3 className="font-bold">{editingId ? 'Edit' : 'Create'} PatientResource</h3>
