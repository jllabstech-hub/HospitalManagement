...
export default function HealthArticleManagement({ initialData }: { initialData: CmsListRecord[] }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [error, setError] = useState('');

  const { register, handleSubmit, reset, setValue, watch, formState: { isSubmitting } } = useForm<CreateHealthArticleInput | UpdateHealthArticleInput>({
  ...
  const onSubmit = async (data: CreateHealthArticleInput) => {
    setError('');
    const res = editingId
      ? await updateHealthArticleAction({ ...data, id: editingId })
      : await createHealthArticleAction(data);
    if (res.success) {
      setEditingId(null);
      reset(emptyForm);
    } else {
    ...
    <div className="space-y-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded border bg-white p-4">
        ...
        <CmsImagePicker
          ...
          description="This image appears on health library listings and article pages."
      ...
      <CmsImageRecordList
        ...
        onEdit={(id) => {
          ...
          reset({ ...record, coverImageUrl: cmsRecordImageUrl(record) || '' } as unknown as CreateHealthArticleInput);
        }}
        onDelete={async (id) => {
          if (confirm('Are you sure you want to delete this record?')) await deleteHealthArticleAction(id);
        }}
      />
    </div>
  );
