...
export default function HealthArticleManagement({ initialData }: { initialData: CmsListRecord[] }) {
  ...
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<CreateHealthArticleInput | UpdateHealthArticleInput>({
  ...
  const editRecord = (record: CmsListRecord) => {
    ...
    reset({ ...record } as unknown as CreateHealthArticleInput);
  };
