...
export default function NewsArticleManagement({ initialData }: { initialData: any[] }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [error, setError] = useState('');
  
  const { register, handleSubmit, reset } = useForm<CreateNewsArticleInput | UpdateNewsArticleInput>({
  ...
  const onSubmit = async (data: any) => {
    ...
    let res;
    if (editingId) {
      res = await updateNewsArticleAction({ ...data, id: editingId });
    } else {
      res = await createNewsArticleAction(data);
    }
    if (res.success) {
      setEditingId(null);
