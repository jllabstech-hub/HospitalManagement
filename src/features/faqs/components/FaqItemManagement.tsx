...
export default function FaqItemManagement({ initialData }: { initialData: CmsListRecord[] }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [error, setError] = useState('');
  
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<CreateFaqItemInput | UpdateFaqItemInput>({
    resolver: zodResolver(editingId ? UpdateFaqItemSchema : CreateFaqItemSchema),
    defaultValues: { question: '', answer: '', category: '' },
  });

  const onSubmit = async (data: CreateFaqItemInput) => {
    setError('');
    let res;
    if (editingId) {
  ...
  const editRecord = (record: CmsListRecord) => {
    ...
    reset({ ...record } as unknown as CreateFaqItemInput);
  };
